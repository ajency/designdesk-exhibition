...
function genesis_sample_woocommerce_theme_notice() {
	...
	$notice_html = sprintf( __( 'Please install and activate <a href="https://wordpress.org/plugins/genesis-connect-woocommerce/" target="_blank">Genesis Connect for WooCommerce</a> to <strong>enable WooCommerce support for %s</strong>.', 'designdesk-child' ), wp_get_theme()->get( 'Name' ) );
	...
	if ( current_user_can( 'install_plugins' ) ) {
		...
		$install_link = sprintf(
			...
			__( 'install and activate Genesis Connect for WooCommerce', 'designdesk-child' )
		);

		/* translators: 1: plugin install prompt presented as link, 2: child theme name */
		$notice_html = sprintf( __( 'Please %1$s to <strong>enable WooCommerce support for %2$s</strong>.', 'designdesk-child' ), $install_link, wp_get_theme()->get( 'Name' ) );
	}
